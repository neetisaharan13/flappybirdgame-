...
SCREENWIDTH = 289
SCREENHEIGHT = 511
# TO INITILISE THE SCREEN
SCREEN = pygame.display.set_mode((SCREENWIDTH, SCREENHEIGHT))
# groud y if the ground in the game ...
...
def welcomeScreen():
    ...
    # whenever a persn presses a key this screen will redirect to main game function
    ...
    while True:
        for event in pygame.event.get():
            # if user clicks on cross button, close the gameSounds
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()
            # if the user presses spaces or up key, start the game for themselves
...
def getRandomPipe():
    '''
    GENERATE POSOTIONS OF TWO PIPES(one bottom straight and one top inverted)
    '''
...
if __name__ == '__main__':
    ...
    pygame.init()  # initilaize all modules of pygane
    FPSCLOCK = pygame.time.Clock()
    pygame.display.set_caption('Flapp bird by Neeti saharan')
